...
/*! \internal
  ...
  imposed, set all Qvector values to Qt's QWIDGETSIZE_MAX.
  
  \a minSizes gives the minimum allowed size of each section. If there shall be no minimum size
  imposed, set all Qvector values to zero. If the \a minSizes entries add up to a value greater than
...
QVector<int> QCPLayout::getSectionSizes(QVector<int> maxSizes, QVector<int> minSizes, QVector<double> stretchFactors, int totalSize) const
{
  ...
  {
    qDebug() << Q_FUNC_INFO << "Passed Qvector sizes aren't equal:" << maxSizes << minSizes << stretchFactors;
    return QVector<int>();
  }
...
void QCPLineEnding::draw(QCPPainter *painter, const QVector2D &pos, const QVector2D &dir) const
{
  ...
  switch (mStyle)
  {
    ...
    case esSkewedBar:
    {
      ...
      {
        // if drawing with cosmetic pen (perfectly thin stroke, happens only in Qvector exports), draw bar exactly on tip of line
...
/*! \fn void QCPAxis::ticksRequest()
  ...
  tick Qvector (and possibly tick label Qvector) once. However, if you want to provide ticks (and
  maybe labels) dynamically, e.g. depending on the current axis range, connect a slot to this
  signal and set the Qvector/Qvectors there.
*/
...
/*!
  ...
  If you need dynamically calculated tick Qvectors (and possibly tick label Qvectors), set the
  Qvectors in a slot connected to the \ref ticksRequest signal.
...
/*!
  ...
  the provided tick Qvector will be overwritten with automatically generated tick coordinates upon
  ...
  \a vec is a Qvector containing the positions of the ticks, in plot coordinates.
...
/*!
  ...
  \ref setTickVector. These two Qvectors should have the same size. (Note that you need to disable
  \ref setAutoTicks and \ref setAutoTickLabels first.)
  
  \a vec is a Qvector containing the labels of the ticks. The entries correspond to the respective
  indices in the tick Qvector, passed via \ref setTickVector.
...
/*! \internal
  ...
  This function is called to prepare the tick Qvector, sub tick Qvector and tick label Qvector. If
  ...
  provide external tick positions. Then the sub tick Qvectors and tick label Qvectors are created.
*/
void QCPAxis::setupTickVectors()
{
  ...
  // fill tick Qvectors, either by auto generating or by notifying user to fill the Qvectors himself
  if (mAutoTicks)
  {
    generateAutoTicks();
  } else
  ...
  } else // mAutoTickLabels == false
  {
    ...
    // make sure provided tick label Qvector has correct (minimal) length:
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  QVector<double> subTickPositions; // the final coordToPixel transformed Qvector passed to QCPAxisPainter
  QVector<double> tickPositions; // the final coordToPixel transformed Qvector passed to QCPAxisPainter
  QVector<QString> tickLabels; // the final Qvector passed to QCPAxisPainter
...
/*! \internal
  ...
  Returns via \a lowIndex and \a highIndex, which ticks in the current tick Qvector are visible in
  the current range. The return values are indices of the tick Qvector, not the positions of the
  ticks themselves.
  
  The actual use of this function is when an external tick Qvector is provided, since it might
...
/*!
  Saves a PDF with the Qvectorized plot to the file \a fileName. The axis ratio as well as the scale
  ...
  \warning
  \li If you plan on editing the exported PDF file with a Qvector graphics editor like
...
/*! \overload
  ...
  Qvectors should have equal length. Else, the number of added points will be the size of the
  smallest Qvector.
*/
...
/*!
  ...
  The provided Qvectors should have equal length. Else, the number of added points will be the size of the
  smallest Qvector.
...
void QCPGraph::draw(QCPPainter *painter)
{
  ...
  // allocate line and (if necessary) point Qvectors:
  QVector<QPointF> *lineData = new QVector<QPointF>;
  QVector<QCPData> *scatterData = 0;
  if (!mScatterStyle.isNone())
    scatterData = new QVector<QCPData>;
  
  // fill Qvectors with data appropriate to plot style:
  ...
  // free allocated line and point Qvectors:
  delete lineData;
  if (scatterData)
    delete scatterData;
}
...
/*! \internal
  ...
  make up steps. If the line style of the graph is \ref lsNone, the \a lineData Qvector will be left
  untouched.
...
/*! \internal
  ...
  scatterData, and the function will skip filling the Qvector.
...
/*! \internal
  ...
  QCPScatterStyle::ssNone. Hence, the \a scatterData Qvector is outputted by all "get(...)PlotData"
...
/*!  \internal
  ...
  connections between the point Qvector they create. These are for example \ref getLinePlotData,
...
/*! \internal
  ...
  The line data Qvector generated by e.g. getLinePlotData contains only the line that connects the
  ...
  because the data Qvector generation functions (getLinePlotData etc.) reserve two extra points when
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  thisData.reserve(lineData->size()+otherData.size()); // because we will join both Qvectors at end of this function
  for (int i=0; i<lineData->size(); ++i) // don't use the Qvector<<(Qvector),  it squeezes internally, which ruins the performance tuning with reserve()
  ...
  // crop both Qvectors to ranges in which the keys overlap (which coord is key, depends on axisType):
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    // x is key
...
/*! \overload
  ...
  provided Qvectors should have equal length. Else, the number of added points will be the size of
  the smallest Qvector.
*/
...
void QCPCurve::setData(const QVector<double> &key, const QVector<double> &value)
{
  ...
  for (int i=0; i<n; ++i)
  {
    newData.t = i; // no t Qvector given, so we assign t the index of the key/value pair
    newData.key = key[i];
    newData.value = value[i];
    mData->insertMulti(newData.t, newData);
  }
...
/*! \internal
  ...
  called by QCPCurve::draw to generate a point Qvector (in pixel coordinates) which represents the
  line of the curve.
...
/*!
  Sets a Qvector of outlier values that will be drawn as scatters. Any data points in the sample
...
void QCPColorMap::draw(QCPPainter *painter)
{
  ...
  // use buffer if painting Qvectorized (PDF):
  bool useBuffer = painter->modes().testFlag(QCPPainter::pmVectorized);
  QCPPainter *localPainter = painter; // will be redirected to paint on mapBuffer if painting Qvectorized
...
/*! \overload
  ...
  Replaces the current data with the provided open/high/low/close data. The provided Qvectors should
  have equal length. Else, the number of added points will be the size of the smallest Qvector.
...
/*! \internal
  ...
  base and the direction Qvector \a vec.
...
/*! \internal
  ...
  Returns the section of the straight line defined by \a base and direction Qvector \a
